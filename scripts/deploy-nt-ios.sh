...
# Deploy Nursing Times to iOS TestFlight
# Usage: ./scripts/deploy-nt-ios.sh [testflight|appstore]
...
echo -e "${BLUE}🚀 Starting Nursing Times iOS deployment to ${TARGET}...${NC}"
...
echo -e "${BLUE}📦 Bundle ID: metropolis.net.nursingtimes${NC}"
...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ Nursing Times iOS deployment to ${TARGET} completed successfully!${NC}"
    
    # Send notification (if configured)
    if [ ! -z "$SLACK_WEBHOOK_URL" ]; then
        curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"✅ Nursing Times iOS deployed to ${TARGET} successfully!\"}" \
            $SLACK_WEBHOOK_URL
    fi
else
    echo -e "${RED}❌ Nursing Times iOS deployment to ${TARGET} failed!${NC}"
    
    # Send failure notification (if configured)
    if [ ! -z "$SLACK_WEBHOOK_URL" ]; then
        curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"❌ Nursing Times iOS deployment to ${TARGET} failed!\"}" \
            $SLACK_WEBHOOK_URL
    fi
