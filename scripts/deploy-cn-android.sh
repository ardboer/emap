...
# Deploy Construction News to Google Play Store
# Usage: ./scripts/deploy-cn-android.sh [internal|alpha|beta|production]
...
echo -e "${BLUE}🚀 Starting Construction News Android deployment to ${TRACK} track...${NC}"
...
echo -e "${BLUE}📦 Package: metropolis.co.uk.constructionnews${NC}"
...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ Construction News Android deployment to ${TRACK} track completed successfully!${NC}"
    
    # Send notification (if configured)
    if [ ! -z "$SLACK_WEBHOOK_URL" ]; then
        curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"✅ Construction News Android deployed to ${TRACK} track successfully!\"}" \
            $SLACK_WEBHOOK_URL
    fi
else
    echo -e "${RED}❌ Construction News Android deployment to ${TRACK} track failed!${NC}"
    
    # Send failure notification (if configured)
    if [ ! -z "$SLACK_WEBHOOK_URL" ]; then
        curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"❌ Construction News Android deployment to ${TRACK} track failed!\"}" \
            $SLACK_WEBHOOK_URL
    fi
